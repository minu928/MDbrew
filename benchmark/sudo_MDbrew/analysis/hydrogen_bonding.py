...
def search_in_H2O(O_position, H_position, box, hb_angle=30.0, hb_distance=3.5):
    ...
    second_O_list = O_tree.query_ball_point(O_position, r=hb_distance)
    ...
    for this_O_position, this_H2_idxes, second_O in zip(
        O_position, H_tree.query(O_position, k=3, p=2, distance_upper_bound=1.2)[1], second_O_list
    ):
        ...
        dist = np.linalg.norm(vec1_arr[idx_of_hydrogen_bonding[0]], axis=-1)
        if dist.any():
